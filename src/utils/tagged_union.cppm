...
private:
    ...
    static constexpr index_t type_index() {
        ...
        for (std::size_t i = 0; i < sizeof...(Types); ++i) {
            if (matches[i]) return static_cast<index_t>(i);
        }
    ...
    template<std::size_t... Is>
    void copy_construct(const TaggedUnion& other, std::index_sequence<Is...> /* indices */) {
        ((other.index_ == Is ? 
            (new(storage_as<type_at<Is>>()) type_at<Is>(*other.storage_as<type_at<Is>>()), void()) : 
            void()), ...);
        index_ = other.index_;
    }
    ...
    void move_construct(TaggedUnion&& other, std::index_sequence<Is... /* indices */>) {
        ((other.index_ == Is ? 
            (new(storage_as<type_at<Is>>()) type_at<Is>(std::move(*other.storage_as<type_at<Is>>())), void()) : 
            void()), ...);
        index_ = other.index_;
...
public:
    ...
    explicit constexpr TaggedUnion(T&& value) {
        ...
        new(storage_as<std::decay_t<T>>()) std::decay_t<T>(std::forward<T>(value));
        index_ = idx;
    }
    ...
    TaggedUnion(TaggedUnion&& other) noexcept {
        move_construct(std::move(other), std::make_index_sequence<sizeof...(Types)>{});
    }
    ...
    [[nodiscard]] constexpr std::size_t index() const noexcept {
        return index_ == static_cast<index_t>(-1) ? npos : static_cast<std::size_t>(index_);
    }
    ...
    template<std::size_t I>
    constexpr auto& get() & {
        if (index_ != I) throw std::bad_variant_access{};
    ...
    constexpr auto visit(Visitor&& vis) & {
        return visit_impl(std::forward<Visitor>(vis), std::make_index_sequence<sizeof...(Types)>{});
    }
...
private:
    ...
    constexpr auto visit_impl(Visitor&& vis, std::index_sequence<Is...> /* indices */) & {
        using return_type = std::common_type_t<decltype(vis(*storage_as<type_at<Is>>()))...>;
        ...
        if (index_ >= sizeof...(Types)) throw std::bad_variant_access{};
        return jump_table[index_](std::forward<Visitor>(vis), storage_);
    }
    
    template<typename Visitor, std::size_t... Is>
    constexpr auto visit_impl(Visitor&& vis, std::index_sequence<Is...> /* indices */) const& {
        using return_type = std::common_type_t<decltype(vis(*storage_as<type_at<Is>>()))...>;
    ...
    template<typename Visitor, std::size_t... Is>
    constexpr auto visit_impl(Visitor&& vis, std::index_sequence<Is...> /* indices */) && {
        using return_type = std::common_type_t<decltype(vis(std::move(*storage_as<type_at<Is>>())))...>;
