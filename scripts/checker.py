...
class CommandExecutor:
    @staticmethod
    def execute(command: str, timeout: int = 30) -> Tuple[str, str, int]:
        try:
...
class OutputComparator:
    ...
    @staticmethod
    def extract_error_lines(stderr: str) -> List[str]:
        lines = []
        for line in stderr.splitlines():
            ...
            elif stripped.startswith("pass failed: "):
                lines.append(stripped)
        return lines

    @staticmethod
    def compare_stderr(
        expected: str, actual: str, scm_file: str, return_code: int
    ) -> bool:
        actual_lines = OutputComparator.extract_error_lines(actual)
        expected_lines = [
            line.replace("%s", scm_file) for line in expected.splitlines()
        ]

        expected_pass_failed = [
            line for line in expected_lines if line.startswith("pass failed: ")
        ]
        expected_errors = [
            line for line in expected_lines if not line.startswith("pass failed: ")
        ]

        actual_pass_failed = [
            line for line in actual_lines if line.startswith("pass failed: ")
        ]
        actual_errors = [
            line for line in actual_lines if not line.startswith("pass failed: ")
        ]

        if actual_errors != expected_errors:
            return False

        if not expected_pass_failed:
            return True

        if actual_pass_failed:
            return actual_pass_failed == expected_pass_failed
...
class TestChecker:
    ...
    def check_file(self, scm_file: str) -> TestResult:
        ...
        actual_stdout, actual_stderr, return_code = self.executor.execute(command)
        actual_stdout = "\n".join(
            [l for l in actual_stdout.splitlines() if not l.startswith("[")]
        )
        ...
        stderr_match = self.comparator.compare_stderr(
            expected_stderr, actual_stderr, scm_file, return_code
        )
    ...
    def _extract_error_lines(stderr_output: str) -> list:
        return OutputComparator.extract_error_lines(stderr_output)
